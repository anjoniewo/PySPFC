...
class CSVexport:
    ...
    def export_node_voltage_plot(self, grid_node_results):
        ...
        self.create_voltage_plot(timestamps=timestamps, y_vals=node_voltages, title="Betrag der Knotenspannung",
    ...
    def create_voltage_plot(self, timestamps=dict(), y_vals=dict(), title="title", x_axis_label="abscissa",
                            y_axis_label="ordinate"):

        settings = self.__settings
        v_nom = 1 if settings.is_import_pu else settings.v_nom
        ...
        if len(y_vals) < MAX_NUM_OF_NODES:
            ...
            for key, value in y_vals.items():
                ...
                plt.plot(timestamps, node_voltages, '-', label=str(key))
